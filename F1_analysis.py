...
def create_leaderboards(cur, results = 10):
    ...
    print('\n')
    print('Lowest Finishing Probability in 2015')
    print('\n')
    for row in cur:
        print(row[0] + ": " + str(round(row[1], 3)*100) + "%")


    cur.execute('SELECT Driver_ids.Name, AVG(Finishing_Position) AS FP FROM Finishes JOIN Driver_ids ON Finishes.Name = Driver_ids.primary_key GROUP BY Driver_ids.Name ORDER BY FP LIMIT {}'.format(results))
    print('\n')
    print('Highest Finishing Drivers in 2015')
    print('\n')
    for row in cur:
